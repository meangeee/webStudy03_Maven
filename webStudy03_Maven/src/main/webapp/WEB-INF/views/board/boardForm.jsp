...
<link rel="stylesheet"
	href="${cPath }/toastmessage/css/jquery.toastmessage.css" />
<script type="text/javascript"
	src="${cPath }/toastmessage/jquery.toastmessage.js"></script>
<link rel="stylesheet"
...
<body>
	<form id="boardForm" method="post" enctype="multipart/form-data">
		<table class="table table-bordered">
			<tr>
				<th>글번호</th>
				<td>${board.bo_no }입력받지 않음. <input type="hidden" name="bo_no"
					value="${board.bo_no }" /> <span class="error">${errors.bo_no }</span></td>
			</tr>
			<tr>
				<th>게시판종류???</th>
				<td><input type="text" required class="form-control"
					name="board_type" value="B01" /> <span class="error">${errors.board_type }</span></td>
			</tr>
			<tr>
				<th>글제목</th>
				<td><input type="text" required class="form-control"
					name="bo_title" value="${board.bo_title }" /> <span class="error">${errors.bo_title }</span></td>
			</tr>
			<tr>
				<th>작성자</th>
				<td><input type="text" required class="form-control"
					name="bo_writer" value="${board.bo_writer }" /> <span
					class="error">${errors.bo_writer }</span></td>
			</tr>
			<tr>
				<th>비밀번호</th>
				<td><input type="password" required class="form-control"
					name="bo_pass" /> <span class="error">${errors.bo_pass }</span></td>
			</tr>
			<tr>
				<th>아이피</th>
				<td><input type="text" required readonly class="form-control"
					name="bo_ip" value="${pageContext.request.remoteAddr }" /> <span
					class="error">${errors.bo_ip }</span></td>
			</tr>
			<tr>
				<th>원글번호</th>
				<td><input type="number" readonly class="form-control"
					name="bo_parent" value="${param.bo_parent }" /> <span
					class="error">${errors.bo_parent }</span></td>
			</tr>
			<c:if test="${not empty board.attatchList }">
				<tr>
					<th>기존 첨부 파일</th>
					<td><c:forEach items="${board.attatchList }" var="attatch"
							varStatus="vs">
							<span> ${attatch.att_filename } <a class="attDelBtn"
								data-attno="${attatch.att_no }"><i
									class="fas fa-minus-square"></i></a> ${not vs.last?",":"" }
							</span>
						</c:forEach></td>
				</tr>
			</c:if>
			<tr>
				...
				<td>
					<div class="form-inline">
						<input type="file" name="bo_file" class="form-control mr-2" /> <a
							class="plusBtn"><span style="font-size: 2em;"><i
			...
			<tr>
				<th>내용</th>
				<td><textarea cols="50" rows="5" name="bo_content">${board.bo_content }</textarea>
			...
			<tr>
				<td colspan="2"><input type="submit" class="btn btn-info"
					...
					value="취소" /> <input type="button" class="btn btn-info"
					value="목록으로"
	...
	<script type="text/javascript">
		...
		$(".plusBtn").on("click", function(event) {
			...
			tdTag.append($("<div>").html($("<input>").attr({
				type : "file",
				name : "bo_file"
			}).addClass("form-control")).addClass("form-inline"));
		});
